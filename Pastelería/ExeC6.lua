...
local function FireServerAndWait(childName)
    game:GetService("ReplicatedStorage").Remotes.TI_0:FireServer(workspace.Ingredients[childName])
end
...
local function HandleHornos(hornoList, waitTime)
    ...
    while true do
        ...
        wait(0.5)
        player.Character.HumanoidRootPart.CFrame = CFrame.new(smokePoint)
        wait(waitTime)
...
for _, horno in ipairs(workspace.Plots.Plot6.Ovens:GetChildren()) do
