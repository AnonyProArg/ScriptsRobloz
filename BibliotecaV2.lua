local opciones = {

    {nombre = "Dex V2", codigo = 'loadstring(game:HttpGet("https://scriptblox.com/raw/Universal-Script-Dex-Explorer-v2-10490"))()'},

    {nombre = "Spy Pc" , codigo = 'loadstring(game:HttpGet("https://raw.githubusercontent.com/78n/SimpleSpy/main/SimpleSpySource.lua"))()'},

    {nombre = "Spy Mobil", codigo = 'loadstring(game:HttpGet("https://raw.githubusercontent.com/HoodProject/Espa-oles/main/Turtle%20Spy.lua"))()'},

    {nombre = "Volar AllGame", codigo = 'loadstring(game:HttpGet("https://raw.githubusercontent.com/Pro69Yes/sussy-Script/main/SecuredFlyGuiv3.lua"))()'},

    {nombre = "AtravesarTodo V.1", codigo = 'loadstring(game:HttpGet("https://raw.githubusercontent.com/AnonyProArg/ScriptsRobloz/main/Nocolision.lua"))()'},

    {nombre = "TPlayer", codigo = 'loadstring(game:HttpGet("https://raw.githubusercontent.com/AnonyProArg/ScriptsRobloz/main/TPplayers.lua"))()'},

    {nombre = "Dex V3", codigo = 'loadstring(game:HttpGet("https://raw.githubusercontent.com/AnonyProArg/ScriptsRobloz/main/Dex%20Explorer.lua.txt"))()'},

    {nombre = "Doors Fluxus", codigo = 'loadstring(game:HttpGet(("https://raw.githubusercontent.com/mstudio45/MSDOORS/main/MSHUB_Loader.lua"),true))()'},

    {nombre = "MeepC GdICasa", codigo = 'loadstring(game:HttpGet("https://raw.githubusercontent.com/AnonyProArg/ScriptsRobloz/main/ColocarYenMeep.lua"))()'},

    {nombre = "RegarPlnts Meepcity", codigo = 'loadstring(game:HttpGet("https://raw.githubusercontent.com/AnonyProArg/ScriptsRobloz/main/plantsMeepC.txt"))()'},

    {nombre = "AutoPesca+ MeepCity", codigo = 'loadstring(game:HttpGet("https://raw.githubusercontent.com/yLegendzz/Scripts/main/Meepcity"))()'},

{nombre = "Doors Delta+", codigo = 'loadstring(game:HttpGet("https://raw.githubusercontent.com/mstudio45/MSDOORS/7bd97c2d956a775d683c2d7973d79715b30998ea/MSDOORS/Moonsec.lua"))()'},

    {nombre = "Teclado Virtual", codigo = 'loadstring(game:HttpGet("https://raw.githubusercontent.com/advxzivhsjjdhxhsidifvsh/mobkeyboard/main/main.txt"))()'},

    {nombre = "Items+ MeepCity", codigo = 'loadstring(game:HttpGet("https://scriptblox.com/raw/MeepCity-Meepcity-cracked-6349"))()'},

    {nombre = "SoloPlus Meepcity", codigo = 'loadstring(game:HttpGet("https://raw.githubusercontent.com/AnonyProArg/gestion/main/PlusMeep"))()'},

    {nombre = "DoubleDown AutoWin", codigo = 'loadstring(game:HttpGet("https://raw.githubusercontent.com/Sw1ndlerScripts/RobloxScripts/main/Misc%20Scripts/DoubleDown.lua?raw=true"))()'},

    {nombre = "Pases DoubleDown", codigo = 'loadstring(game:HttpGet("https://raw.githubusercontent.com/AnonyProArg/ScriptsRobloz/main/Pases%20DoubleDown.lua"))()'},

    {nombre = "TP2", codigo = 'loadstring(game:HttpGet("https://raw.githubusercontent.com/AnonyProArg/ScriptsRobloz/main/TP2.lua"))()'},

    {nombre = "Tycoon-Milk", codigo = 'loadstring(game:HttpGet("https://raw.githubusercontent.com/AnonyProArg/ScriptsRobloz/main/TpTycoonMilk.lua"))()'loadstring(game:HttpGet("https://raw.githubusercontent.com/AnonyProArg/ScriptsRobloz/main/Tycoon%20Milk.lua"))()'},

    {nombre = "TP Botones T-Milk", codigo = 'loadstring(game:HttpGet("https://raw.githubusercontent.com/AnonyProArg/ScriptsRobloz/main/TpTycoonMilk.lua"))()'},

    {nombre = "Opción 6", codigo = 'loadstring(game:HttpGet("###"))()'},

{nombre = "Opción 2", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 3", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 4", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 5", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 6", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 2", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 3", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 4", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 5", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 6", codigo = 'loadstring(game:HttpGet("###"))()'},

{nombre = "Opción 2", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 3", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 4", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 5", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 6", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 2", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 3", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 4", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 5", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 6", codigo = 'loadstring(game:HttpGet("###"))()'},

{nombre = "Opción 2", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 3", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 4", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 5", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 6", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 2", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 3", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 4", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 5", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 6", codigo = 'loadstring(game:HttpGet("###"))()'},

{nombre = "Opción 2", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 3", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 4", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 5", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 6", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 2", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 3", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 4", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 5", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 6", codigo = 'loadstring(game:HttpGet("###"))()'},

{nombre = "Opción 2", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 3", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 4", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 5", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 6", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 2", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 3", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 4", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 5", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 6", codigo = 'loadstring(game:HttpGet("###"))()'},

{nombre = "Opción 2", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 3", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 4", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 5", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 6", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 2", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 3", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 4", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 5", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 6", codigo = 'loadstring(game:HttpGet("###"))()'},

{nombre = "Opción 2", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 3", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 4", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 5", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 6", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 2", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 3", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 4", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 5", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 6", codigo = 'loadstring(game:HttpGet("###"))()'},

{nombre = "Opción 2", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 3", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 4", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 5", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 6", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 2", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 3", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 4", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 5", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 6", codigo = 'loadstring(game:HttpGet("###"))()'},

{nombre = "Opción 2", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 3", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 4", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 5", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 6", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 2", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 3", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 4", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 5", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 6", codigo = 'loadstring(game:HttpGet("###"))()'},

{nombre = "Opción 2", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 3", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 4", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 5", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 6", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 2", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 3", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 4", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 5", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 6", codigo = 'loadstring(game:HttpGet("###"))()'},

{nombre = "Opción 2", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 3", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 4", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 5", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 6", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 2", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 3", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 4", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 5", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 6", codigo = 'loadstring(game:HttpGet("###"))()'},

{nombre = "Opción 2", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 3", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 4", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 5", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 6", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 2", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 3", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 4", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 5", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 6", codigo = 'loadstring(game:HttpGet("###"))()'},

{nombre = "Opción 2", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 3", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 4", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 5", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 6", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 2", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 3", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 4", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 5", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 6", codigo = 'loadstring(game:HttpGet("###"))()'},

{nombre = "Opción 2", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 3", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 4", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 5", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 6", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 2", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 3", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 4", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 5", codigo = 'loadstring(game:HttpGet("###"))()'},

    {nombre = "Opción 6", codigo = 'loadstring(game:HttpGet("###"))()'},

}

-- Crear un objeto ScreenGui

local gui = Instance.new("ScreenGui")

gui.Parent = game.Players.LocalPlayer:WaitForChild("PlayerGui")

-- Crear un objeto Frame para el menú flotante

local frame = Instance.new("Frame")

frame.Size = UDim2.new(0, 500, 0, 300)

frame.Position = UDim2.new(0.5, -250, 0.5, -150)

frame.BackgroundColor3 = Color3.new(1, 1, 1)

frame.BorderSizePixel = 2

frame.Parent = gui

-- Crear un objeto ScrollingFrame para contener los botones

local scrollingFrame = Instance.new("ScrollingFrame")

scrollingFrame.Size = UDim2.new(1, -10, 1, -10)

scrollingFrame.Position = UDim2.new(0, 5, 0, 5)

scrollingFrame.BackgroundTransparency = 1

scrollingFrame.ScrollBarThickness = 6

scrollingFrame.Parent = frame

-- Crear un objeto GridLayout para organizar los botones en el ScrollingFrame

local gridLayout = Instance.new("UIGridLayout")

gridLayout.CellSize = UDim2.new(0, 120, 0, 30)

gridLayout.CellPadding = UDim2.new(0, 10, 0, 10)

gridLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center

gridLayout.VerticalAlignment = Enum.VerticalAlignment.Top

gridLayout.SortOrder = Enum.SortOrder.LayoutOrder

gridLayout.Parent = scrollingFrame

-- Crear los botones en el ScrollingFrame

for i, opcion in ipairs(opciones) do

    local button = Instance.new("TextButton")

    button.Size = UDim2.new(1, 0, 1, 0)

    button.BackgroundColor3 = Color3.new(0, 0.7, 0)

    button.TextColor3 = Color3.new(1, 1, 1)

    button.Text = opcion.nombre

    button.LayoutOrder = i

    button.Parent = scrollingFrame

    -- Función para ejecutar el código de la opción seleccionada y cerrar el menú

    local function ejecutarCodigo()

        loadstring(opcion.codigo)()

        frame:Destroy()  -- Cerrar el menú flotante

    end

    -- Conectar la función al evento MouseButton1Click del botón

    button.MouseButton1Click:Connect(ejecutarCodigo)

end

local isMobile = system.getInfo("platform") == "android" or system.getInfo("platform") == "ios"

-- Función para simular eventos de puntero del mouse

local function simulateMousePointer(event)

    if isMobile then

        -- Obtener las coordenadas táctiles

        local touchX, touchY = event.x, event.y

        

        -- Simular eventos de puntero del mouse

        if event.phase == "began" then

            system.activate("mouse")

            system.setMouseFocus(true)

            system.moveMouse(touchX, touchY)

            

        elseif event.phase == "moved" then

            system.moveMouse(touchX, touchY)

            

        elseif event.phase == "ended" then

            system.activate("mouse")

            system.setMouseFocus(false)

        end

    end

    return true  -- Indica que el evento ha sido manejado correctamente

end

-- Manejo de eventos táctiles

local function handleTouchEvents()

    if isMobile then

        Runtime:addEventListener("touch", simulateMousePointer)

    end

end
local closeButton = Instance.new("TextButton")

closeButton.Parent = screenGui

closeButton.Size = UDim2.new(0, 150, 0, 50)

closeButton.Position = UDim2.new(0.85, -75, 0.2, 0)

closeButton.BackgroundColor3 = Color3.new(0.5, 0.5, 0.5)

closeButton.Text = "Cerrar Menú"

-- Función para cerrar el menú

local function closeMenu()

    screenGui:Destroy()

end

-- Conectar función al evento Click del botón de cerrar menú

closeButton.Activated:Connect(closeMenu)

-- Llamar a la función para manejar los eventos táctiles

handleTouchEvents()


